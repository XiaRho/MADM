...
from checkpoint.odise_checkpointer import LdmCheckpointer
from modeling.meta_arch.clip import ClipAdapter
...
class LdmExtractor(FeatureExtractor):
    def __init__(
        ...
        **kwargs
        ...
        if ldm is not None:
            ...
            self.ldm = LatentDiffusion(**kwargs)
        if enable_resize:
    ...
    def forward(self, batched_inputs):
        ...
        # else:
        #     normalized_image = self.image_preprocess(image)
        assert -1 <= torch.min(normalized_image) and torch.max(normalized_image) <= 1
        ...
        latent_image, encoder_features = self.encode_to_latent(normalized_image)  # input: -1~1
        ...
        for i, t in enumerate(self.steps):
            ...
            # noisy_latent_image: [2, 4, 64, 64] tensor(-7.0792, device='cuda:0')~tensor(4.8985, device='cuda:0')
            # torch.Size([1, 4, 64, 64]) tensor([0], device='cuda:0') torch.Size([1, 77, 768]) torch.Size([1, 1280])
...
class PositionalLinear(nn.Module):
    ...
    def forward(self, x):
        x = self.linear(x)  # [1, 768] --> [1, 768]
        x = x.unsqueeze(1) + self.positional_embedding  # [1, 1, 768] + [1, 77, 768] = [1, 77, 768]
        return x


class ClipFeatureProject(nn.Module):
    def __init__(self, learnable_cond_prompt=False, prompt_in_features=None, prompt_out_features=None, prompt_seq_len=None, # prompt_alpha_cond_size=None, 
                 learnable_cond_time=False, time_in_features=None, time_out_features=None, time_seq_len=None, time_alpha_cond_size=None,
                 input_prefix=False, without_prompt_alpha=True, multi_layer_prompt=False, init_uncond_prompt=False, uncond_prompt=None):
        super().__init__()

        self.learnable_cond_prompt = learnable_cond_prompt
        self.learnable_cond_time = learnable_cond_time
        self.input_prefix = input_prefix
        self.without_prompt_alpha = without_prompt_alpha
        self.multi_layer_prompt = multi_layer_prompt
        self.init_uncond_prompt = init_uncond_prompt

        if self.learnable_cond_prompt:
            if not self.input_prefix:
                self.prompt_embed = torch.zeros(1, prompt_seq_len, prompt_out_features)
                if self.init_uncond_prompt:
                    self.prompt_embed = F.interpolate(uncond_prompt[None], size=(self.prompt_embed.shape[-2:]), mode='bilinear', align_corners=False)[0]
                if self.multi_layer_prompt:
                    self.prompt_embed = self.prompt_embed[None].repeat(16, 1, 1, 1)
                if not self.init_uncond_prompt:
                    trunc_normal_(self.prompt_embed, std=0.02)
                self.prompt_embed = nn.Parameter(self.prompt_embed)
            else:
                self.prompt_embed_project = PositionalLinear(prompt_in_features, prompt_out_features, prompt_seq_len)

            if not self.without_prompt_alpha:
                # assert not self.input_prefix
                if self.multi_layer_prompt:
                    alpha_shape = [16, 1, prompt_seq_len, prompt_out_features]
                else:
                    alpha_shape = [1, prompt_seq_len, prompt_out_features]
                self.alpha_cond_prompt = nn.Parameter(torch.rand(alpha_shape))
                self.alpha_uncond_prompt = nn.Parameter(torch.rand(alpha_shape))

        if self.learnable_cond_time:
            self.alpha_cond_time = nn.Parameter(torch.zeros(time_alpha_cond_size))
            if not self.input_prefix:
                self.time_embed = nn.Parameter(torch.zeros(1, time_seq_len, time_out_features))
                trunc_normal_(self.time_embed, std=0.02)
            else:
                self.time_embed_project = PositionalLinear(time_in_features, time_out_features, time_seq_len)

    def get_cond_prompt(self, uncond_prompt, prefix=None):
        if self.learnable_cond_prompt:
            if not self.without_prompt_alpha and uncond_prompt.shape[1] != self.alpha_cond_prompt.shape[1]:
                uncond_prompt = F.interpolate(uncond_prompt[None], size=(self.alpha_cond_prompt.shape[-2:]), mode='bilinear', align_corners=False)[0]
            if not self.input_prefix:
                if not self.without_prompt_alpha:
                    cond_prompt = torch.tanh(self.alpha_uncond_prompt) * uncond_prompt + torch.tanh(self.alpha_cond_prompt) * self.prompt_embed
                else:
                    cond_prompt = self.prompt_embed
            else:
                if not self.without_prompt_alpha:
                    cond_prompt = torch.tanh(self.alpha_uncond_prompt) * uncond_prompt + torch.tanh(self.alpha_cond_prompt) * self.prompt_embed_project(prefix)
                else:
                    cond_prompt = self.prompt_embed_project(prefix)
        else:
            return uncond_prompt
        return cond_prompt
                
    # def get_cond_prompt(self, uncond_prompt=None, prefix=None):
    #     if self.learnable_cond_prompt:
    #         if not self.input_prefix:
    #             cond_prompt = torch.tanh(self.alpha_cond_prompt) * self.prompt_embed
    #         else:
    #             cond_prompt = torch.tanh(self.alpha_cond_prompt) * self.prompt_embed_project(prefix)
    #         return cond_prompt
    #     else:
    #         return uncond_prompt
        
    def get_cond_time(self, prefix=None):
        if self.learnable_cond_time:
            if not self.input_prefix:
                cond_time = torch.tanh(self.alpha_cond_time) * self.time_embed
            else:
                # cond_time will not affect the frad of self.clip
                cond_time = torch.tanh(self.alpha_cond_time) * self.time_embed_project(prefix.detach())
        else:
            cond_time = None
        return cond_time
    
    def forward(self, uncond_prompt, prefix=None):
        cond_prompt = self.get_cond_prompt(uncond_prompt, prefix)
        cond_time = self.get_cond_time(prefix)
        return cond_prompt, cond_time


class BasePromptTimeGenerator(nn.Module):

    cross_attention_out_dim = [320, 320, 640, 640, 1280, 1280, 1280, 1280, 1280, 1280, 640, 640, 640, 320, 320, 320]

    def __init__(
        self,
        learnable_cond_prompt=True,
        learnable_cond_time=True,
        same_cond_params=False,
        detach_prompt_for_mixed_data=False,
        clip_state="no",
        num_timesteps=1,
        clip_model_name="",  # ViT-L-14
        ldm_extractor=None,

        without_prompt_alpha=False,
        multi_layer_prompt=False,
        mix_source_target_prompt=False,
        init_uncond_prompt=False,
        mask_prompt_ratio=False,
        detach_mask_prompt=False,
        ...
        **kwargs,
        ...
        self.learnable_cond_prompt = learnable_cond_prompt
        self.learnable_cond_time = learnable_cond_time
        self.same_cond_params = same_cond_params
        self.detach_prompt_for_mixed_data = detach_prompt_for_mixed_data
        self.clip_state = clip_state
        assert self.clip_state in {'no', 'learnable_clip', 'no_learnable_clip'}
        self.multi_layer_prompt = multi_layer_prompt
        self.without_prompt_alpha = without_prompt_alpha
        self.mix_source_target_prompt = mix_source_target_prompt
        self.init_uncond_prompt = init_uncond_prompt
        self.mask_prompt_ratio = mask_prompt_ratio
        self.detach_mask_prompt = detach_mask_prompt
        assert not (self.detach_mask_prompt and (not self.mask_prompt_ratio))
        ...
        if ldm_extractor is None:
            ...
            alpha_cond_time_embed_shape = self.ldm_extractor.ldm.unet.time_embed[-1].out_features
            self.uncond_inputs = self.ldm_extractor.ldm.uncond_inputs.cuda().detach()
        else:
            self.ldm_extractor = ldm_extractor
            self.text_embed_shape = self.ldm_extractor.text_embed_shape
            time_embed_project_out_feats = self.ldm_extractor.unet_time_embed_out_features
            alpha_cond_time_embed_shape = self.ldm_extractor.unet_time_embed_out_features
            self.uncond_inputs = self.ldm_extractor.uncond_inputs.cuda().detach()

        if self.clip_state != 'no':
            self.clip = ClipAdapter(name=clip_model_name, normalize=False, requires_grad=(self.clip_state == 'learnable_clip'))
            prompt_in_features, time_in_features = self.clip.dim_latent, self.clip.dim_latent
        else:
            prompt_in_features, time_in_features = None, None

        if 'prompt_seq_len' in kwargs.keys():
            prompt_seq_len = kwargs['prompt_seq_len']
        else:
            prompt_seq_len = self.text_embed_shape[0]
        
        # ################################################
        # ###### learnable parms for SD model (RGB) ######
        # ################################################
        self.clip_project_rgb = ClipFeatureProject(
            learnable_cond_prompt=self.learnable_cond_prompt, 
                prompt_in_features=prompt_in_features, 
                prompt_out_features=self.text_embed_shape[1], 
                prompt_seq_len=prompt_seq_len, 
                # prompt_alpha_cond_size=torch.Size([1, prompt_seq_len, self.text_embed_shape[1]]), 
                without_prompt_alpha=self.without_prompt_alpha,
                multi_layer_prompt=self.multi_layer_prompt,
                init_uncond_prompt=self.init_uncond_prompt,
                uncond_prompt=self.uncond_inputs,
            learnable_cond_time=self.learnable_cond_time,
                time_in_features=time_in_features, 
                time_out_features=time_embed_project_out_feats, 
                time_seq_len=num_timesteps, 
                time_alpha_cond_size=alpha_cond_time_embed_shape,
            input_prefix=(self.clip_state != 'no')
        )
        
        # ############################################################
        # ###### learnable parms for SD model (second modality) ######
        # ############################################################
        if self.same_cond_params:
            self.clip_project_others = self.clip_project_rgb
        else:
            self.clip_project_others = ClipFeatureProject(
                learnable_cond_prompt=self.learnable_cond_prompt, 
                    prompt_in_features=prompt_in_features, 
                    prompt_out_features=self.text_embed_shape[1], 
                    prompt_seq_len=prompt_seq_len, 
    ...
    def forward(self, batched_inputs, input_modal, ema_forward=False, timestep=None, return_unet_feats=False, **kwargs):
        """
        Args:
        ...
        """
        assert input_modal in {'rgb', 'others', 'mixed', 'masked_prompt', 'prompt_perturbation', 'rand_prompt'}

        image = batched_inputs["img"]  # [batch_size, 3, 512, 512]  0~1

        # parepare clip prefix
        if self.clip_state != 'no':
            if self.clip_state == 'learnable_clip':
                if ema_forward:
                    prefix = self.ema_clip.embed_image(image).image_embed  # [batch_size, 768]
                else:
                    prefix = self.clip.embed_image_with_grad(image).image_embed  # [batch_size, 768]
            else:
        ...
        # if input_modal == 'rgb':
        #     batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = self.clip_project_rgb(uncond_prompt=self.uncond_inputs, prefix=None)
        # else:
        #     # parepare clip prefix
        #     prefix = None
        #     if self.clip_state == 'learnable_clip':
        ...
        #         else:
        #             prefix = self.clip.embed_image_with_grad(image).image_embed  # [batch_size, 768]
        #     elif self.clip_state == 'no_learnable_clip':
        #         prefix = self.clip.embed_image(image).image_embed

        #     clip_project = self.ema_clip_project_others if ema_forward else self.clip_project_others
        #     batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = clip_project(self.uncond_inputs, prefix)
        #     if input_modal == 'mixed':
        #         if self.uncond_inputs.shape[1] != batched_inputs["cond_inputs"].shape[1]:
        #             mixed_uncond_inputs = F.interpolate(self.uncond_inputs[None], size=(batched_inputs["cond_inputs"].shape[-2:]), mode='bilinear', align_corners=False)[0]
        #         else:
        #             mixed_uncond_inputs = self.uncond_inputs
        #         batched_inputs["cond_inputs"] = 0.5 * batched_inputs["cond_inputs"] + 0.5 * mixed_uncond_inputs

        if input_modal == 'rgb':
            assert ema_forward is False
            batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = self.clip_project_rgb(self.uncond_inputs, prefix)
        elif input_modal == 'mixed' and self.mix_source_target_prompt:
            s_cond_inputs, s_cond_emb = self.clip_project_rgb(self.uncond_inputs, prefix)
            t_cond_inputs, t_cond_emb = self.clip_project_others(self.uncond_inputs, prefix)
            batched_inputs["cond_inputs"] = (s_cond_inputs + t_cond_inputs) / 2
            batched_inputs["cond_emb"] = (s_cond_emb + t_cond_emb) / 2
        else:
            clip_project = self.ema_clip_project_others if ema_forward else self.clip_project_others
            batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = clip_project(self.uncond_inputs, prefix)

        if input_modal == 'mixed' and self.detach_prompt_for_mixed_data:
            batched_inputs["cond_inputs"] = batched_inputs["cond_inputs"].detach()

        if input_modal == 'masked_prompt' and self.mask_prompt_ratio:
            if self.detach_mask_prompt:
                with torch.no_grad():
                    batched_inputs["cond_inputs"] = self.mask_prompt(batched_inputs["cond_inputs"]).detach()
            else:
                batched_inputs["cond_inputs"] = self.mask_prompt(batched_inputs["cond_inputs"])
        elif input_modal == 'prompt_perturbation' and self.prompt_perturbation:
            rand_noise = torch.randn(batched_inputs["cond_inputs"].shape, device=batched_inputs["cond_inputs"].device) * self.prompt_perturbation
            batched_inputs["cond_inputs"] += rand_noise
            batched_inputs["cond_inputs"] = batched_inputs["cond_inputs"].detach()
        elif input_modal == 'rand_prompt':
            batched_inputs["cond_inputs"] = torch.rand_like(batched_inputs["cond_inputs"]) * self.rand_prompt_scale

        assert not return_unet_feats
        # if return_unet_feats:
        #     batched_inputs["return_unet_feats"] = True
        #     return self.ldm_extractor(batched_inputs)[1]
        # else:

        if timestep is not None:
            batched_inputs['timestep'] = timestep

        # repeat if bs != 1
        if image.shape[0] != 1: 
            batched_inputs["cond_inputs"] = torch.repeat_interleave(batched_inputs["cond_inputs"], repeats=image.shape[0], dim=0)
            batched_inputs["cond_emb"] = torch.repeat_interleave(batched_inputs["cond_emb"], repeats=image.shape[0], dim=0)

        if input_modal == 'prompt_perturbation' and self.prompt_perturbation:
            with torch.no_grad():
                out = self.ldm_extractor(batched_inputs, input_modal, ema_forward=ema_forward, **kwargs)
        else:
            out = self.ldm_extractor(batched_inputs, input_modal, ema_forward=ema_forward, **kwargs)
        return out

    def mask_prompt(self, prompt):
        if not self.multi_layer_prompt:
            assert prompt.dim() == 3
            prompt = prompt[None]
        layer_num, batch_size, length, feat_dim = prompt.shape
        input_mask = torch.rand((layer_num, batch_size, length, 1), device=prompt.device)
        input_mask = (input_mask > self.mask_prompt_ratio).float()
        input_mask = input_mask.repeat(1, 1, 1, feat_dim)
        if not self.multi_layer_prompt:
            prompt = prompt[0]
        assert input_mask.shape == prompt.shape

        return prompt * input_mask
    ...
    def extra_repr(self):
        return f"learnable_time_embed={self.learnable_cond_time}"
