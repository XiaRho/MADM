...
class ClipFeatureProject(nn.Module):
    def __init__(self, in_features, out_features, seq_len, alpha_cond_size, learnable_time_embed,
                 time_in_features=0, time_out_features=0, time_seq_len=0, time_alpha_cond_size=0,
                 without_prompt=False):
        super().__init__()

        self.without_prompt = without_prompt
        if not self.without_prompt:
            self.clip_project = PositionalLinear(in_features, out_features, seq_len)
            self.alpha_cond = nn.Parameter(torch.zeros_like(alpha_cond_size))
        ...
        if self.learnable_time_embed:
            ...
            self.alpha_cond_time_embed = nn.Parameter(torch.zeros(time_alpha_cond_size))

    def forward(self, prefix, uncond_inputs):
        cond_inputs = uncond_inputs
        if not self.without_prompt:
            cond_inputs += torch.tanh(self.alpha_cond) * self.clip_project(prefix)
        if self.learnable_time_embed:
            cond_emb = torch.tanh(self.alpha_cond_time_embed) * self.time_embed_project(prefix)
        else:
            cond_emb = None
        return cond_inputs, cond_emb
...
class LdmImplicitCaptionerExtractor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if ldm_extractor is None:
            ...
            self.uncond_inputs = self.ldm_extractor.uncond_inputs.cuda()
        ...
        self.clip_project_rgb = ClipFeatureProject(
            in_features=self.clip.dim_latent, out_features=self.text_embed_shape[1], seq_len=self.text_embed_shape[0],
            alpha_cond_size=self.uncond_inputs, learnable_time_embed=self.learnable_time_embed,
            time_in_features=self.clip.dim_latent, time_out_features=time_embed_project_out_feats,
            time_seq_len=num_timesteps, time_alpha_cond_size=alpha_cond_time_embed_shape,
            without_prompt=self.without_prompt
        )

        # ############################################################
        # ###### learnable parms for SD model (second modality) ######
        # ############################################################
        self.clip_project_others = deepcopy(self.clip_project_rgb)
    ...
    def forward(self, batched_inputs, input_modal, ema_forward=False, return_unet_feats=False):
        """
        Args:
        ...
        prefix = self.clip.embed_image(image).image_embed  # [batch_size, 768]

        if input_modal == 'rgb':
            assert ema_forward is False
            batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = self.clip_project_rgb(prefix, self.uncond_inputs)
        else:
            clip_project = self.ema_clip_project_others if ema_forward else self.clip_project_others
            batched_inputs["cond_inputs"], batched_inputs["cond_emb"] = clip_project(prefix, self.uncond_inputs)
...
class TextAdapter(nn.Module):
    def __init__(self, text_dim=768, hidden_dim=None, gamma_init_value=1e-4):
        super().__init__()
        assert text_dim == 768
    ...
    def forward(self, texts, bs=1):
        if texts.shape[0] == 1 and bs != 1:
            texts = texts[0]
        texts_after = self.fc(texts)  # [77, 768] --> [77, 768]
