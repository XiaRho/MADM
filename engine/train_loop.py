...
class SimpleTrainer(_SimpleTrainer):
    def __init__(self, model, data_loader, optimizer, *, grad_clip=None):
        ...
        # self.logger.warning(f"Trainer: {self.__class__.__name__}")
        # self.logger.warning(f"grad_clip: {grad_clip}")
        # self.logger.warning("All parameters: \n" + parameter_count_table(model))

        # # print trainable parameters
        # self.logger.warning("Trainable parameters: \n" + parameter_count_table(model, trainable_only=True, max_depth=8))
    ...
    def run_step(self):
        ...
        with autocast():
            ...
            if isinstance(loss_dict, torch.Tensor):
                ...
                losses = sum(loss_dict.values())  # forward
