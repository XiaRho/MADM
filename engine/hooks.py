...
class VisHook(_EvalHook):
    def __init__(self, vis_period, vis_function):
        super().__init__(vis_period, vis_function)
        func_args = inspect.getfullargspec(vis_function).args
        assert {"final_iter", "next_iter"}.issubset(set(func_args)), (
            f"Eval function must have either 'final_iter' or 'next_iter' as an argument."
            f"Got {func_args} instead."
        )
        ...
        if results:
            ...
            for k, v in flattened_results.items():
                try:
                    ...
                    raise ValueError(
                        "[VisHook] vis_function should return a nested dict of float. "
                        "Got '{}: {}' instead.".format(k, v)
                    ) from e
            self.trainer.storage.put_scalars(**flattened_results, smoothing_hint=False)
